# 🕹️ Evaluacio-Victor-Orellana

Proyecto final — Evaluación Unidad I · Programación Web  
Carrera: Informática · Instituto Profesional Santo Tomás

---

## 📁 Estructura del proyecto

```
Evaluacion-VictorOrellana/
├── index.html        ← Página principal
├── gracias.html      ← Página de agradecimiento (formulario)
├── css/
│   └── styles.css    ← Todos los estilos (separado del HTML)
├── js/
│   └── main.js       ← JavaScript (hamburguesa, validación formulario)
├── img/
│   ├── xbox-switch.jpg
│   ├── nintendo-switch.jpg
│   └── ps5.jpg
├── videos/
│   ├── video1.mp4    ← Video local 1
│   └── video2.mp4    ← Video local 2
└── README.md
```

---

## ✅ Requisitos cumplidos

- [x] Desarrollado en HTML5 y CSS3 (CSS separado)
- [x] 3 secciones de productos con imagen y título
- [x] 3 videos (2 locales + 1 iframe YouTube)
- [x] Formulario de contacto centrado con validación
- [x] Página de agradecimiento con botón volver
- [x] Diseño responsive (mobile-first)
- [x] Compatible Chrome, Firefox, Safari, Edge

---

URL del proyecto: `https://[tu-usuario].github.io/estacion-arcade/`
